/**
* The RBCFeed object is the primary conduit to the data feed. It provides
...
* to follow. This object knows nothing about the UI, see RBCFeed-ui.js for
* how this object is used to make sure the UI is updated as events come in.
*
* @param    {string}    baseURL     The _firebase URL.
* @param    {boolean}   newContext  Whether a new _firebase context is used.
...
this._firebase = new Firebase(baseURL, newContext || false ? new _firebase.Context() : null);
...
return "https://graph.facebook.com/" + (id || this._userid) + "/picture/?type=" + ( large ? "large" : "square") + "&return_ssl_resources=1";
},
...
*/
RBCFeed.prototype.onLoginStateChange = function(onLoginStateChange) {
var self = this;
...
/**
* Login a user using _firebase Simple Login, using the specified authentication
* provider. Pass the optional 'rememberMe' argument to the _firebaseSimpleLogin
...
this._firebaseAuthClient.login('password', {
email : username,
password : password
});
};
...
RBCFeed.prototype.createAccount = function(provider, username, password, firstname, lastname, bio, location) {
	var newData = (this._firebase);
	console.log(this._firebase);
this._firebaseAuthClient.createUser(username, password, function(error, user) {
if (!error) {
	...
	newData.child('people').child(user.id).set({fullName: firstname + lastname, name: firstname, bio: bio, location: location}, onComplete);
...
*/
RBCFeed.prototype.logout = function() {
if (this._userid) {
...
/**
* On successful authentication, set up _firebase references and hang on to
* relevant user data like id and name. _firebase Simple Login automatically
...
*/
RBCFeed.prototype.onLogin = function(user) {
...
* the current user as offline. _firebase Simple Login automatically handles
* user sessions, so there is no need to do any additional sessioning here.
*/
RBCFeed.prototype.onLogout = function() {
this._user = null;
this._userid = null;
this._mainUser = null;
this._fullName = null;
...
*/
RBCFeed.prototype.getUserInfo = function(user, onComplete, onFollower, onFollowersComplete, onFollowee, onFolloweesComplete) {
var self = this;
...
if (onFolloweesComplete)
onFolloweesComplete();
});
};

RBCFeed.prototype.startSearch = function(resultsHandler) {
this._searchHandler = resultsHandler;
};

RBCFeed.prototype.updateSearchTerm = function(term) {
...
this._currentSearch = new RBCFeedSearch(this._firebase, term, this._searchHandler);
}
} else {
this.stopSearching();
}
...
RBCFeed.prototype.stopSearching = function() {
if (this._currentSearch) {
...
*/
RBCFeed.prototype.getSpark = function(id, onComplete) {
var self = this;
self._validateCallback(onComplete, true);
...
*/
RBCFeed.prototype.follow = function(user, onComplete) {
var self = this;
...
*/
RBCFeed.prototype.post = function(content, onComplete) {
var self = this;
...
*                                        suggested user.
*/
RBCFeed.prototype.getSuggestedUsers = function(onSuggestedUser) {
var self = this;
...
*/
RBCFeed.prototype.setProfileField = function(field, value) {
var peopleRef = this._firebase.child("people").child(this._userid);
...
* spark (see RBCFeed.post), which will appear in real-time on the current
* user's feed!
*
...
*/
RBCFeed.prototype.onNewSpark = function(totalCount, onComplete, onOverflow) {
...
*/
RBCFeed.prototype.onNewSparkFor = function(id, count, onComplete, onOverflow) {
...
*                                   latest set.
*/
RBCFeed.prototype.onLatestSpark = function(count, onComplete, onOverflow) {
this._validateCallback(onComplete, true);
this._validateCallback(onOverflow, true);
...
* reuse the RBCFeed object after calling this and registering new handlers.
*/
RBCFeed.prototype.unload = function() {
